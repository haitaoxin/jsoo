...
既然函数不是普通的对象，那必定有它特别之处。表象之下，关键是它比其它对象类型多了一个内部特征值（internal property）叫做`[[Call]]`。内部特征值被JavaScript语言定义于标准对象类型上，它们是无法被我们的代码读写，但是JavaScript引擎会使用它们来正确处理不同种类的对象。内部特征值通常用双方括号`[[ ]]`标识，今后我们还会见到。而`[[Call]]`这个特征值只有函数这种对象才有，JavaScript引擎的`typeof`运算符也是靠检查这个值来正确识别`function`这个类型的。当然有个这个特征值，JavaScript引擎也就可以调用一个函数了。
