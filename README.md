...
在JavaScript里，“对象（object）”一词其实有两层含义。狭义的内涵是指我们用花括弧标识的N对 key-value 值（N 大于等于 0）。而当我们说“所有的引用数据类型都是对象”的时候，这里的“对象”是取其广义，是指狭义的对象概念加上其它各种内建的对象类型，即便这些内建类型不一定是被括弧包括的 k-v 对，比如函数、函数、Map、Set等等。
### 构建对象（Object）
